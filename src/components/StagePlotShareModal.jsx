...
import { createStagePlotShare, getStagePlotShare, revokeStagePlotShare } from '../lib/stagePlotShare';
import { useToast } from './ui/Toast';

export default function StagePlotShareModal({ open, onClose, eventId }) {
  ...
  useEffect(() => {
    if (!open) return;
    getStagePlotShare(eventId).then(setShare).catch((error) => showToast(error.message, 'error'));
  }, [eventId, open, showToast]);

  async function create() {
    setBusy(true);
    try {
      setShare(await createStagePlotShare(eventId, expiration));
