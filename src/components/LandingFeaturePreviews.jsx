// Illustrative previews for the landing page's "See it in action" showcase
// — same approach as LandingDashboardPreview: hand-built to match the app's
// real visual language (badges, card style, browser chrome) rather than
// generic stock mockups, since there are no real product screenshots to use
// here. Sample data only.
  ...
  return (
    <BrowserChrome label="app.gigworks.io/bookings/chen-martinez">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="min-w-0 truncate text-sm font-semibold text-slate-800">Chen–Martinez Wedding</div>
      ...
      <div className="grid grid-cols-4 gap-1 text-[10px] text-slate-400 -mt-2 mb-4 px-0.5">
        {PIPELINE_STAGES.map((stage) => <span key={stage} className="truncate text-center first:text-left last:text-right">{stage}</span>)}
      </div>
