    ...
    <Modal open={open} onClose={onClose} title="Email Prep Sheet" widthClass="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {prepContractors.map((c) => (
                ...
                >
                  <input
                    ...
                    data-testid="prep-email-recipient-checkbox"
                    className="sr-only"
                  />
                  {c.name}
                </label>
        ...
        <div>
          ...
          <input value={subject} onChange={(e) => setSubject(e.target.value)} data-testid="prep-email-subject-input" className={inputClass} />
        </div>
        ...
        <div>
          ...
          <div
            ...
            data-testid="prep-email-body-input"
            ...
            <div className="space-y-1">
              {documents.map((d) => (
                <label key={d.id} className="flex items-center gap-2 text-sm text-slate-600">
                  <input type="checkbox" checked={documentIds.includes(d.id)} onChange={() => toggleDocument(d.id)} data-testid="prep-email-document-checkbox" />
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button type="button" onClick={onClose} data-testid="prep-email-cancel-button" className="px-4 py-2 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-100">
            Cancel
          </button>
          <button
            type="submit"
            disabled={sending || !subject.trim() || !hasBody || recipientIds.length === 0}
            data-testid="prep-email-send-button"
