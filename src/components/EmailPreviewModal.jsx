  ...
  return (
    <Modal open={open} onClose={onClose} title={`Preview email${recipientLabel ? ` — ${recipientLabel}` : ''}`} widthClass="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        {note && (
          <div data-testid="email-preview-note-banner" className="text-xs text-slate-500 bg-slate-50 border border-slate-100 rounded-lg px-3 py-2">{note}</div>
        )}

        <div>
          <label className={labelClass}>Subject</label>
          <input value={subject} onChange={(e) => setSubject(e.target.value)} data-testid="email-preview-subject-input" className={inputClass} />
        </div>
        ...
        <div>
          ...
          <div
            ...
            data-testid="email-preview-body-input"
            className={bodyEditableClass}
          />
        </div>

        <div className="flex justify-end gap-2 pt-2">
          <button type="button" onClick={onClose} data-testid="email-preview-cancel-button" className="px-4 py-2 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-100">
            Cancel
          </button>
          <button
            type="submit"
            disabled={sending || !subject.trim() || !hasBody}
            data-testid="email-preview-send-button"
