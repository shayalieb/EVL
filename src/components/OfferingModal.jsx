      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {form.type === 'package' && (
          <div className="space-y-4 rounded-xl border border-indigo-100 bg-indigo-50/40 p-4">
            ...
            <div className="flex items-center justify-between"><div><p className="text-sm font-bold text-slate-800">Package line items</p><p className="text-xs text-slate-500">Included items use the package price. Additional options can have their own price and quantity.</p></div><button type="button" onClick={() => update('lineItems', [...form.lineItems, blankLine()])} className="rounded-lg border border-indigo-300 bg-white px-3 py-2 text-xs font-semibold text-indigo-700">+ Add line item</button></div>
            <div className="space-y-3">
              ...
              {form.lineItems.map((item, index) => {
                ...
                return <div key={item.id} className="rounded-lg border border-slate-200 bg-white p-3 space-y-3">
                  ...
                  {item.excludedFromPrice && <div className="grid gap-2 sm:grid-cols-4"><label className={labelClass}>Pricing<select value={item.pricingType} onChange={(e) => patchLine({ pricingType: e.target.value })} className={inputClass}><option value="flat">Flat price</option><option value="perUnit">Per unit</option></select></label><label className={labelClass}>QTY<input type="number" min="0" value={item.quantity ?? 1} onChange={(e) => patchLine({ quantity: e.target.value })} className={inputClass} /></label>{item.pricingType === 'perUnit' && <label className={labelClass}>Unit<select value={item.unitType} onChange={(e) => patchLine({ unitType: e.target.value })} className={inputClass}>{UNITS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>}<label className={labelClass}>{item.pricingType === 'flat' ? 'Option price' : 'Price per unit'}<MoneyInput value={item.rate} onChange={(value) => patchLine({ rate: value })} className={moneyInputClass} /></label></div>}
                  <div className="flex flex-wrap gap-x-5 gap-y-2">{!item.excludedFromPrice && <label className="flex items-center gap-2 text-xs font-semibold text-slate-600"><input type="checkbox" checked={item.required} onChange={(e) => patchLine({ required: e.target.checked, selected: e.target.checked || item.selected })} />Always show this included item</label>}<label className="flex items-center gap-2 text-xs font-semibold text-amber-700"><input type="checkbox" checked={Boolean(item.excludedFromPrice)} onChange={(e) => patchLine({ excludedFromPrice: e.target.checked, required: e.target.checked ? false : item.required })} />Additional option — priced separately</label></div>
                </div>;
              })}
            </div>
          </div>
