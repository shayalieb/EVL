...
export default function ConfirmDialog({ open, onClose, onConfirm, title = 'Are you sure?', description, confirmLabel = 'Delete', danger = true, confirmText }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={title} widthClass="max-w-sm" descriptionId={description ? descriptionId : undefined}>
      <p id={description ? descriptionId : undefined} className="text-sm text-slate-600 mb-3">{description}</p>
      {requiresTyping && (
        <div className="mb-5">
      ...
      <div className="flex justify-end gap-2">
        <button
          ...
          className="min-h-11 px-4 py-2 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-100"
        >
          Cancel
        </button>
        <button
          ...
          className={`min-h-11 px-4 py-2 rounded-lg text-sm font-semibold text-white disabled:opacity-40 disabled:cursor-not-allowed ${danger ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}
        >
          {confirmLabel}
        </button>
      </div>
