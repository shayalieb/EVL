...
function PackageConfigForm({ offering, onAdd, onCancel }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="max-h-[26rem] space-y-3 overflow-y-auto pr-1">{instance.lineItems.map((item) => <div key={item.id} className={`rounded-xl border p-3 ${item.selected ? 'border-indigo-200 bg-indigo-50/30' : 'border-slate-200 bg-slate-50 opacity-70'}`}>
      <div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-slate-800">{item.name}</p><p className={`text-xs ${item.excludedFromPrice ? 'font-semibold text-amber-700' : 'text-slate-500'}`}>{item.excludedFromPrice ? `Additional option · ${item.pricingType === 'perUnit' ? `${currency(item.rate)} per ${item.unitType}` : currency(item.rate)}` : 'Included in package price'}</p></div><label className="flex items-center gap-1.5 text-xs font-semibold text-slate-600"><input type="checkbox" checked={item.selected !== false} disabled={item.required} onChange={(e) => updateLine(item.id, { selected: e.target.checked })} />{item.required ? 'Included' : 'Show'}</label></div>
      {item.excludedFromPrice && item.selected !== false && <label className="mt-3 block text-xs font-semibold text-slate-500">QTY{item.pricingType === 'perUnit' ? ` (${item.unitType}s)` : ''}<input type="number" min="0" value={item.quantity ?? 1} onChange={(e) => updateLine(item.id, { quantity: e.target.value })} className={`${inputClass} mt-1`} /></label>}
    </div>)}</div>
