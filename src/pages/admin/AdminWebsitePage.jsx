...
const TABS = [
  ...
  ['features', 'Features'], ['coming-soon', 'Coming Soon'], ['agency', 'Agency Tier'], ['reviews', 'Reviews & Stories'], ['pricing', 'Pricing'], ['faq', 'FAQ'], ['forms', 'Forms & Footer'], ['legal', 'Legal Details'], ['privacy', 'Privacy Policy'], ['terms', 'Terms of Service'],
];
...
export default function AdminWebsitePage() {
  ...
  function updateMessaging(key, value) { updateSection('pricing', 'messaging', { ...config.pricing.messaging, [key]: value }); }
  function updateMessagingPackage(index, key, value) { updateMessaging('packages', config.pricing.messaging.packages.map((item, i) => i === index ? { ...item, [key]: value } : item)); }
  ...
  function addComingSoonItem() {
    updateSection('comingSoon', 'items', [...config.comingSoon.items, { header: 'Upcoming feature', description: 'Describe what customers can look forward to.' }]);
  }
  function removeComingSoonItem(index) {
    updateSection('comingSoon', 'items', config.comingSoon.items.filter((_, i) => i !== index));
  }
  function addReview() {
  ...
  return (
    <form onSubmit={save} className="max-w-6xl space-y-5">
      ...
      {tab === 'coming-soon' && <div className="space-y-5">
        <Card title="Coming soon section">
          <div className="flex items-start justify-between gap-4"><p className="text-sm text-slate-500 max-w-2xl">Publish upcoming features without making them look available today. Turn the entire section off whenever you do not need it.</p><label className="flex items-center gap-2 text-sm font-semibold text-slate-700 shrink-0"><input type="checkbox" checked={config.comingSoon.enabled} onChange={(e) => updateSection('comingSoon', 'enabled', e.target.checked)} />Section live</label></div>
          <div className="grid sm:grid-cols-2 gap-3"><Field label="Section label" maxLength={80} value={config.comingSoon.label} onChange={(v) => updateSection('comingSoon', 'label', v)} /><Field label="Heading" maxLength={160} value={config.comingSoon.heading} onChange={(v) => updateSection('comingSoon', 'heading', v)} /></div>
          <Field label="Section description" rows={3} value={config.comingSoon.description} onChange={(v) => updateSection('comingSoon', 'description', v)} />
        </Card>
        <div className="grid md:grid-cols-2 gap-5">{config.comingSoon.items.map((item, index) => <Card key={index} title={`Coming soon item ${index + 1}`}><Field label="Header" maxLength={140} value={item.header} onChange={(v) => updateNested('comingSoon', 'items', index, 'header', v)} /><Field label="Description" rows={4} value={item.description} onChange={(v) => updateNested('comingSoon', 'items', index, 'description', v)} /><button type="button" onClick={() => removeComingSoonItem(index)} disabled={config.comingSoon.items.length <= 1} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-red-600 hover:bg-red-50 disabled:opacity-30 disabled:cursor-not-allowed">Remove item</button></Card>)}</div>
        <button type="button" onClick={addComingSoonItem} disabled={config.comingSoon.items.length >= 20} className="px-4 py-2 rounded-lg border border-indigo-200 text-sm font-semibold text-indigo-700 hover:bg-indigo-50 disabled:opacity-40 disabled:cursor-not-allowed">+ Add line item</button>
      </div>}
      ...
      {tab === 'pricing' && <div className="space-y-5"><Card title="Pricing section"><div className="grid sm:grid-cols-2 gap-3"><Field label="Section label" maxLength={60} value={config.pricing.label} onChange={(v) => updateSection('pricing', 'label', v)} /><Field label="Heading" maxLength={140} value={config.pricing.heading} onChange={(v) => updateSection('pricing', 'heading', v)} /></div><Field label="Description" value={config.pricing.description} onChange={(v) => updateSection('pricing', 'description', v)} /><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3"><Field label="Monthly label" maxLength={30} value={config.pricing.monthlyLabel} onChange={(v) => updateSection('pricing', 'monthlyLabel', v)} /><Field label="Annual label" maxLength={30} value={config.pricing.annualLabel} onChange={(v) => updateSection('pricing', 'annualLabel', v)} /><Field label="Savings badge" maxLength={60} value={config.pricing.annualSavingsLabel} onChange={(v) => updateSection('pricing', 'annualSavingsLabel', v)} /><Field label="Featured badge" maxLength={40} value={config.pricing.featuredLabel} onChange={(v) => updateSection('pricing', 'featuredLabel', v)} /></div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3"><Field label="Per-month suffix" maxLength={30} value={config.pricing.perMonthLabel} onChange={(v) => updateSection('pricing', 'perMonthLabel', v)} /><Field label="Annual billing label" maxLength={50} value={config.pricing.billedAnnuallyLabel} onChange={(v) => updateSection('pricing', 'billedAnnuallyLabel', v)} /><Field label="Monthly billing label" maxLength={50} value={config.pricing.monthlyLabel} onChange={(v) => updateSection('pricing', 'billedMonthlyLabel', v)} /><Field label="Trial button ({days} supported)" maxLength={80} value={config.pricing.trialButtonLabel} onChange={(v) => updateSection('pricing', 'trialButtonLabel', v)} /><Field label="Trial footer ({days} supported)" maxLength={100} value={config.pricing.trialFooterLabel} onChange={(v) => updateSection('pricing', 'trialFooterLabel', v)} /></div><div className="grid sm:grid-cols-2 gap-3"><Field label="Trial days" type="number" min="0" max="60" value={config.pricing.trialDays} onChange={(v) => updateSection('pricing', 'trialDays', v)} /><Field label="Pricing footer" maxLength={300} value={config.pricing.footer} onChange={(v) => updateSection('pricing', 'footer', v)} /></div><StringList label="Features included in every plan" maxItems={12} items={config.pricing.includedFeatures} onChange={(items) => updateSection('pricing', 'includedFeatures', items)} /></Card><Card title="Contractor Messaging"><p className="text-sm text-slate-500">These allowances and packages appear on the public pricing section. SMS and WhatsApp are delivery channels that share one message allowance.</p><div className="grid sm:grid-cols-2 gap-3"><Field label="Package heading" maxLength={160} value={config.pricing.messaging.heading} onChange={(v) => updateMessaging('heading', v)} /><Field label="Availability badge" maxLength={100} value={config.pricing.messaging.availabilityLabel} onChange={(v) => updateMessaging('availabilityLabel', v)} /></div><Field label="Package description" rows={2} value={config.pricing.messaging.description} onChange={(v) => updateMessaging('description', v)} /><Field label="Usage and overage note" rows={2} value={config.pricing.messaging.usageNote} onChange={(v) => updateMessaging('usageNote', v)} /><div className="grid sm:grid-cols-3 gap-3">{config.pricing.messaging.packages.map((item, index) => <div key={index} className="rounded-xl border border-slate-200 p-3 space-y-2"><Field label="Messages" type="number" min="1" max="100000" value={item.texts} onChange={(v) => updateMessagingPackage(index, 'texts', Number(v))} /><Field label="Monthly $" type="number" min="1" step="0.01" value={(item.monthlyAmountCents / 100).toFixed(2)} onChange={(v) => updateMessagingPackage(index, 'monthlyAmountCents', Math.round(Number(v) * 100))} /></div>)}</div></Card><div className="grid md:grid-cols-3 gap-5">{config.pricing.tiers.map((tier, index) => <Card key={tier.id} title={tier.name}><Field label="Plan name" maxLength={40} value={tier.name} onChange={(v) => updateTier(index, 'name', v)} /><div className="grid grid-cols-2 gap-2"><Field label="Monthly $" type="number" min="1" step="0.01" value={(tier.monthlyAmountCents / 100).toFixed(2)} onChange={(v) => updateTierDollars(index, 'monthlyAmountCents', v)} /><Field label="Annual $" type="number" min="1" step="0.01" value={(tier.annualAmountCents / 100).toFixed(2)} onChange={(v) => updateTierDollars(index, 'annualAmountCents', v)} /></div><Field label={tier.id === 'agency' ? 'Messages included monthly per group' : 'Messages included monthly'} type="number" min="0" max="1000000" value={tier.includedTexts} onChange={(v) => updateTier(index, 'includedTexts', Number(v))} /><Field label="Description" rows={3} value={tier.description} onChange={(v) => updateTier(index, 'description', v)} /><label className="flex items-center gap-2 text-sm text-slate-600"><input type="checkbox" checked={tier.featured} onChange={(e) => updateTier(index, 'featured', e.target.checked)} />Featured plan</label></Card>)}</div><p className="text-xs text-slate-500">Changing an amount creates a new Stripe price for future customers. Existing subscribers keep their current rate.</p></div>}
