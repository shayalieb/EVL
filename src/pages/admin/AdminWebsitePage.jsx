    ...
    <form onSubmit={save} className="max-w-6xl space-y-5">
      ...
      {tab === 'features' && <div className="space-y-5">
        ...
        {config.features.comparison.categories.map((category, categoryIndex) => <Card key={category.id} title={category.name}>
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead><tr className="text-left text-xs text-slate-500"><th className="pb-2">Feature</th><th className="pb-2">Solo</th><th className="pb-2">Team</th><th className="pb-2">Studio</th><th /></tr></thead><tbody>{category.rows.map((row, rowIndex) => <tr key={row.id}><td className="pr-2 pb-2"><input className={inputClass} value={row.feature} onChange={(e) => updateComparisonCategory(categoryIndex, (current) => ({ ...current, rows: current.rows.map((item, index) => index === rowIndex ? { ...item, feature: e.target.value } : item) }))} /></td>{['solo', 'team', 'studio'].map((tier) => <td key={tier} className="px-1 pb-2"><input className={inputClass} value={row[tier]} onChange={(e) => updateComparisonCategory(categoryIndex, (current) => ({ ...current, rows: current.rows.map((item, index) => index === rowIndex ? { ...item, [tier]: e.target.value } : item) }))} /></td>)}<td className="pl-2 pb-2"><button type="button" onClick={() => updateComparisonCategory(categoryIndex, (current) => ({ ...current, rows: current.rows.filter((_, index) => index !== rowIndex) }))} className="text-red-500 hover:text-red-700" aria-label={`Remove ${row.feature}`}>×</button></td></tr>)}</tbody></table></div>
