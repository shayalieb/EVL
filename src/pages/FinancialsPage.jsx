...
function ReportTable({ headers, children, empty }) {
  return <div className="hidden overflow-x-auto sm:block"><table className="w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-400"><tr>{headers.map((header) => <th key={header.label} className={`px-4 py-3 ${header.right ? 'text-right' : ''}`}>{header.label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{children || <tr><td colSpan={headers.length} className="px-5 py-10 text-center text-slate-400">{empty}</td></tr>}</tbody></table></div>;
}
...
function ReceivablesReport({ report }) {
  ...
  return <><div className="grid grid-cols-2 md:grid-cols-5 gap-3 p-5">{buckets.map(([label, amount]) => <Metric key={label} label={label} value={amount} color={label === 'Current' ? 'text-slate-700' : 'text-amber-700'} />)}</div><div className="divide-y divide-slate-100 border-t border-slate-100 sm:hidden">{report.rows.length === 0 ? <p className="px-5 py-10 text-center text-sm text-slate-400">No outstanding client balances.</p> : report.rows.map((row) => <article key={row.invoiceId} className="space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><Link to={`/bookings/${row.bookingId}?tab=invoices`} className="font-semibold text-indigo-600">{row.clientName}</Link><p className="truncate text-xs text-slate-400">{row.bookingName}</p></div><p className="shrink-0 font-bold text-slate-700">{money(row.balance)}</p></div><dl className="grid grid-cols-2 gap-2 text-xs"><div><dt className="font-semibold uppercase tracking-wide text-slate-400">Invoice</dt><dd className="mt-0.5 text-slate-600">#{row.invoiceNumber ?? '—'}</dd></div><div><dt className="font-semibold uppercase tracking-wide text-slate-400">Due date</dt><dd className="mt-0.5 text-slate-600">{row.dueDate ? new Date(row.dueDate).toLocaleDateString() : 'Not set'}</dd></div></dl><p className={`text-xs font-semibold ${row.overdueDays > 0 ? 'text-rose-600' : 'text-slate-500'}`}>{row.overdueDays > 0 ? `${row.overdueDays} days overdue` : 'Current'}</p></article>)}</div><ReportTable headers={[{ label: 'Client / booking' }, { label: 'Invoice' }, { label: 'Due date' }, { label: 'Age' }, { label: 'Balance', right: true }]} empty="No outstanding client balances.">{report.rows.length > 0 && report.rows.map((row) => <tr key={row.invoiceId}><td className="px-4 py-3"><Link to={`/bookings/${row.bookingId}?tab=invoices`} className="font-semibold text-indigo-600 hover:text-indigo-700">{row.clientName}</Link><p className="text-xs text-slate-400">{row.bookingName}</p></td><td className="px-4 py-3 text-slate-500">#{row.invoiceNumber ?? '—'}</td><td className="px-4 py-3 text-slate-500">{row.dueDate ? new Date(row.dueDate).toLocaleDateString() : 'No due date'}</td><td className={`px-4 py-3 ${row.overdueDays > 0 ? 'font-semibold text-rose-600' : 'text-slate-500'}`}>{row.overdueDays > 0 ? `${row.overdueDays} days overdue` : 'Current'}</td><td className="px-4 py-3 text-right font-bold text-slate-700">{money(row.balance)}</td></tr>)}</ReportTable></>;
}
...
export default function FinancialsPage() {
  ...
  return (
    <main className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-8 space-y-6">
      ...
      <AcceptPaymentModal open={!!payingContractor} title={payingContractor ? `Pay ${payingContractor.contractorName}` : 'Mark contractor paid'} confirmLabel="Mark paid" amountDue={payingContractor?.amount ?? payingContractor?.expectedAmount} amountLabel="Expected payment" paymentDetails={payingContractor} initialValues={payingContractor ? { method: payingContractor.preferredPaymentMethod || '', checkNumber: payingContractor.preferredPaymentMethod === 'venmo' ? payingContractor.venmoHandle || '' : payingContractor.preferredPaymentMethod === 'zelle' ? payingContractor.zelleContact || '' : '' } : undefined} onClose={() => setPayingContractor(null)} onAccept={markContractorPaid} />
    </main>
  );
}
