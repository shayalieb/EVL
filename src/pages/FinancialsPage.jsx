  ...
  return (
    <main className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-8 space-y-6">
      ...
      {loading && !summary ? <div className="py-20 text-center text-sm text-slate-400">Loading financials…</div> : summary && <>
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm flex flex-wrap items-center gap-x-6 gap-y-1">
          <p className="text-sm font-bold text-slate-800">Expected in the next 30 days</p>
          <p className="text-sm"><span className="font-semibold text-emerald-700">+{money(summary.next30.expectedIn)} in</span><span className="text-slate-300 mx-2">·</span><span className="font-semibold text-rose-700">−{money(summary.next30.expectedOut)} out</span><span className="text-slate-300 mx-2">·</span><span className={`font-bold ${summary.next30.net >= 0 ? 'text-indigo-700' : 'text-rose-700'}`}>net {money(summary.next30.net)}</span></p>
        </section>
