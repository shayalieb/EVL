  ...
  return (
    <PublicShell>
      <header className="mb-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm print:shadow-none">
        ...
        <p className="mt-3 text-xs text-slate-400">Revision {data.share.revision?.revisionNumber || '—'} · Published {data.share.revision?.publishedAt ? new Date(data.share.revision.publishedAt).toLocaleString() : new Date(stagePlot.updatedAt).toLocaleString()}{data.share.revision?.publishedByName ? ` by ${data.share.revision.publishedByName}` : ''}</p>
        {data.share.revision?.note && <p className="mt-2 rounded-lg bg-indigo-50 px-3 py-2 text-sm text-indigo-800">Update note: {data.share.revision.note}</p>}
      </header>
