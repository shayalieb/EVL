            ...
            <div className="space-y-1">
              ...
              {selectedGig.paymentStatus === 'paid' ? (
                ...
                <div data-testid="contractor-gig-detail-payment-unpaid" className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                  ...
                  {selectedGig.paymentRequest && <div className="mt-3 border-t border-amber-200 pt-3 text-xs text-slate-700"><p><strong>Status:</strong> {selectedGig.paymentRequest.status === 'disputed' ? 'Needs an update' : selectedGig.paymentRequest.status === 'approved' ? 'Approved—awaiting payment' : 'Sent to the business for review'}</p><p className="mt-1"><strong>Requested:</strong> {currency(selectedGig.paymentRequest.amount)}</p>{selectedGig.paymentRequest.invoiceNumber && <p className="mt-1"><strong>Invoice/reference:</strong> {selectedGig.paymentRequest.invoiceNumber}</p>}{selectedGig.paymentRequest.note && <p className="mt-1 whitespace-pre-wrap"><strong>Note:</strong> {selectedGig.paymentRequest.note}</p>}{selectedGig.paymentRequest.reviewNote && <p className="mt-2 rounded bg-white/70 p-2 text-rose-700"><strong>Please update:</strong> {selectedGig.paymentRequest.reviewNote}</p>}</div>}
                </div>
              )}
            </div>
