...
export default function FloorPlanEditorPage() {
  ...
  function handlePageSaved(pageId, patch) {
    setPlan((prev) => (prev ? { ...prev, pages: prev.pages.map((pg) => (pg.id === pageId ? { ...pg, ...patch } : pg)) } : prev));
  }

  async function handleAddPage() {
    const page = await addFloorPlanPage(eventId);
    setPlan((prev) => ({ ...prev, pages: [...prev.pages, page] }));
    setActivePageId(page.id);
  }

  async function handleDeletePage(pageId) {
    if (!plan || plan.pages.length <= 1) return;
    await deleteFloorPlanPage(eventId, pageId);
    setPlan((prev) => ({ ...prev, pages: prev.pages.filter((p) => p.id !== pageId) }));
    setActivePageId((prev) => (prev === pageId ? plan.pages.find((p) => p.id !== pageId)?.id || null : prev));
  }

  async function handleExportPdf() {
    setExporting(true);
    try {
      await generateFloorPlanPdf({ eventId, eventName: event?.name, floorPlan: plan, businessInfo: currentUser?.businessInfo });
    } finally {
      setExporting(false);
    }
  }

  if (loadError) return <div data-testid="floorplan-load-error" className="p-6 text-sm text-red-600">{loadError}</div>;
  if (!plan) return <div className="p-6 text-sm text-slate-500">Loading…</div>;

  const sortedPages = plan.pages.slice().sort((a, b) => a.order - b.order);
    ...
    <div className="p-6 max-w-[1500px] mx-auto">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <h1 className="text-lg font-bold text-slate-800">Floor Plan{event?.name ? ` — ${event.name}` : ''}</h1>
        </div>
        <button
          type="button"
          onClick={handleExportPdf}
          disabled={exporting}
          data-testid="floorplan-export-pdf-button"
      ...
      <div className="flex items-center gap-1 mb-3 border-b border-slate-200">
        {sortedPages.map((p) => (
          <button
            ...
            data-testid="floorplan-page-tab"
        ...
        <button type="button" onClick={handleAddPage} data-testid="floorplan-add-page-button" className="px-3 py-2 text-sm text-indigo-600 font-semibold">
          + Page
        </button>
        {sortedPages.length > 1 && activePage && (
          <button
            ...
            data-testid="floorplan-delete-page-button"
