              ...
              {o.type === 'package' ? (
                <div className="space-y-2">
                  {(o.lineItems || []).map((item) => <div key={item.id} className={`rounded-lg border p-3 ${item.selected === false ? 'border-slate-200 bg-slate-50 opacity-70' : 'border-indigo-100 bg-indigo-50/30'}`}>
                    <div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-slate-700">{item.name}</p><p className={`text-xs ${item.excludedFromPrice ? 'font-semibold text-amber-700' : 'text-slate-400'}`}>{item.excludedFromPrice ? `Additional option · ${item.pricingType === 'perUnit' ? `${currency(item.rate)} per ${item.unitType}` : currency(item.rate)}` : 'Included in package price'}</p></div><label className="flex items-center gap-1.5 text-xs font-semibold text-slate-600"><input type="checkbox" checked={item.selected !== false} disabled={item.required} onChange={(e) => handleUpdate(o.id, { lineItems: o.lineItems.map((line) => line.id === item.id ? { ...line, selected: e.target.checked } : line) })} />{item.required ? 'Included' : 'Show'}</label></div>
                    {item.excludedFromPrice && item.selected !== false && <label className="mt-2 block text-[11px] text-slate-400">QTY{item.pricingType === 'perUnit' ? ` (${item.unitType}s)` : ''}<input type="number" min="0" value={item.quantity ?? 1} onChange={(e) => handleUpdate(o.id, { lineItems: o.lineItems.map((line) => line.id === item.id ? { ...line, quantity: e.target.value } : line) })} className={`${inputClass} mt-1`} /></label>}
                  </div>)}
