...
const syncLabels = { synced: 'Synced', ready: 'Ready', mismatch: 'Review needed', needs_customer: 'Match customer', not_eligible: 'Not issued', missing_client: 'Client missing', setup_required: 'Setup required', failed: 'Retry needed' };
const syncColors = { synced: 'bg-emerald-100 text-emerald-700', ready: 'bg-blue-100 text-blue-700', mismatch: 'bg-amber-100 text-amber-800', needs_customer: 'bg-amber-100 text-amber-800', failed: 'bg-red-100 text-red-700', not_eligible: 'bg-slate-100 text-slate-500', missing_client: 'bg-red-100 text-red-700', setup_required: 'bg-slate-100 text-slate-500' };
...
const billLabels = { ready: 'Ready', synced: 'Synced', mismatch: 'Review needed', failed: 'Retry needed', needs_vendor: 'Vendor first', missing_rate: 'Rate missing', not_confirmed: 'Not confirmed' };
...
export default function IntegrationsTab() {
  ...
  return <div className="max-w-3xl space-y-5">
    <section className="rounded-xl border border-slate-200 bg-white p-5">
      ...
      {connected ? <div className="mt-5 rounded-lg border border-emerald-200 bg-emerald-50 p-4"><p className="font-semibold text-slate-800">{connection.companyName || 'QuickBooks company'}</p><p className="mt-1 text-xs text-slate-500">Connected securely. You remain in control: review each accounting record before synchronizing it.</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={checkConnection} disabled={working} className="rounded-lg border border-emerald-300 bg-white px-3 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 disabled:opacity-50">{working ? 'Checking…' : 'Check connection'}</button><button type="button" onClick={disconnect} disabled={working} className="rounded-lg border border-red-200 bg-white px-3 py-2 text-sm font-semibold text-red-600 hover:bg-red-50 disabled:opacity-50">Disconnect</button></div></div> : <div className="mt-5"><button type="button" onClick={connect} disabled={working || !connection?.configured || !connection?.accessEnabled} className="rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50">{working ? 'Opening QuickBooks…' : connection?.status === 'needs_reauthorization' ? 'Reconnect QuickBooks' : 'Connect QuickBooks'}</button>{!connection?.accessEnabled ? <p className="mt-2 text-xs text-amber-700">QuickBooks is currently available to approved pilot accounts. Contact GigWorks to request access.</p> : !connection?.configured && <p className="mt-2 text-xs text-amber-700">QuickBooks connection will become available after the Intuit production credentials are configured.</p>}</div>}
    </section>
    ...
    <section className="rounded-xl border border-blue-100 bg-blue-50 p-5"><h3 className="font-bold text-slate-800">Safe QuickBooks onboarding</h3><ol className="mt-3 space-y-2 text-sm text-slate-600"><li><strong>1.</strong> Connect the correct QuickBooks company and run the connection check.</li><li><strong>2.</strong> Import accounts and confirm every accounting mapping before syncing.</li><li><strong>3.</strong> Review customer and vendor matches to prevent duplicates.</li><li><strong>4.</strong> Start with one invoice, payment, contractor bill, and contractor payment.</li><li><strong>5.</strong> Verify the results in QuickBooks, then enable reconciliation monitoring.</li></ol><p className="mt-4 text-xs text-blue-800">GigWorks never deletes QuickBooks records when you disconnect. Corrections, reversals, and uncertain matches remain review-based.</p></section>
