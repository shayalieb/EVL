...
const painItems = [
  ...
  ['Contractor conversations disappear into personal phones.', 'Confirmations, schedule changes, and payment follow-ups get scattered across separate text threads with no shared record for the office.', 'Send contractor texts from the gig, keep the conversation in Contact History, and see the context alongside the booking.'],
...
const featureGroups = [
  ...
  ['communication', 'bolt', 'For contractor communication', ['SMS and WhatsApp actions directly on the contractor strip', 'Plan-based monthly text allowances', 'Contact History keeps outreach connected to the gig', 'Usage alerts before the monthly allowance is reached']],
...
const faqItems = [
  ...
  ['Is contractor texting included in my plan?', 'Yes. Solo includes 100 texts per month, Team includes 500, and Studio includes 1,500. Agency includes 500 texts per managed group. Messaging becomes available when SMS and WhatsApp launch; email remains included separately.'],
  ['What happens if I need more texts?', 'You can add a monthly text package without changing plans: 500 additional texts for $15, 1,000 for $25, or 2,500 for $50. GigWorks warns account owners as usage approaches the monthly limit and does not automatically create surprise overage charges.'],
  ['What counts as one text?', 'A normal-length SMS counts as one text. Longer SMS messages can be split into more than one text by mobile carriers, and GigWorks shows the count before you send. Regular WhatsApp messages use the same monthly message allowance when WhatsApp is enabled.'],
...
const comparisonCategories = [
  ...
  ['Contractor messaging', [
    ['Monthly texts included', '100', '500', '1,500'],
    ['SMS from the contractor strip', 'Included', 'Included', 'Included'],
    ['WhatsApp messaging', 'Included', 'Included', 'Included'],
  ...
  name,
  rows: rows.map(([feature, solo, team, studio], rowIndex) => ({ id: `feature-${categoryIndex + 1}-${rowIndex + 1}`, feature, solo, team, studio, agency: feature === 'Team members' ? 'Custom' : feature === 'Monthly texts included' ? '500 per group' : 'Included' })),
}));
...
export const DEFAULT_WEBSITE_CONFIG = {
  ...
  comingSoon: { enabled: true, label: 'Coming soon', heading: 'More ways to run the gig from one place', description: 'These additions are being prepared for launch. Availability and timing may change as we finish testing.', items: [{ header: 'SMS and WhatsApp messaging', description: 'Message contractors directly from the contractor strip, with plan-based monthly text allowances and conversations recorded in Contact History.' }] },
  pricing: { label: 'Simple pricing', heading: 'Every plan runs the whole gig', description: 'Core tools and a monthly contractor-text allowance are built into every plan. Choose based on the size and structure of your operation.', monthlyLabel: 'Monthly', annualLabel: 'Annual', annualSavingsLabel: 'Save up to 20%', featuredLabel: 'Most popular', perMonthLabel: '/month', billedAnnuallyLabel: 'billed annually', billedMonthlyLabel: 'Billed monthly', trialButtonLabel: 'Start {days}-day free trial', trialFooterLabel: '{days}-day free trial at launch.', trialDays: 14, footer: 'Cancel anytime. Secure billing through Stripe. Messaging requires activation and will be available when SMS and WhatsApp launch.', includedFeatures: ['Unlimited clients, bookings, and events', 'Proposals, e-sign contracts, and invoices', 'Contractor confirmations and availability', 'Stage plots, set lists, and production details', 'Templates, reminders, and client payment tools'], messaging: { heading: 'Need more texts? Add them without changing plans.', description: 'Choose a monthly text package whenever your team needs more contractor communication.', availabilityLabel: 'SMS & WhatsApp launching soon', usageNote: 'No surprise overages. Account owners receive alerts at 75%, 90%, and 100%, then choose whether to add more texts.', packages: [{ texts: 500, monthlyAmountCents: 1500 }, { texts: 1000, monthlyAmountCents: 2500 }, { texts: 2500, monthlyAmountCents: 5000 }] }, tiers: [...PLAN_TIERS.map((tier) => ({ id: tier.id, name: tier.label, seatLimit: tier.seatLimit, includedTexts: tier.id === 'solo' ? 100 : tier.id === 'team' ? 500 : 1500, monthlyAmountCents: tier.monthly.amountCents, annualAmountCents: tier.annual.amountCents, description: tier.id === 'solo' ? 'For independent bandleaders and performers running their own calendar.' : tier.id === 'team' ? 'For small teams sharing bookings, staffing, and client follow-up.' : 'For growing entertainment companies coordinating multiple people.', featured: tier.id === 'team' })), { id: 'agency', name: 'Agency', seatLimit: null, includedTexts: 500, includedTextsPerGroup: true, monthlyAmountCents: 14900, annualAmountCents: 142800, includedGroupCount: 2, monthlyAdditionalGroupCents: 3500, annualAdditionalGroupCents: 33600, description: 'For management companies coordinating multiple bands or acts, each with its own workflow and brand.', featured: false }] },
