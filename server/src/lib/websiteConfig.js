...
const painItems = [
  ...
  ['Contractor conversations disappear into personal phones.', 'Confirmations, schedule changes, and payment follow-ups get scattered across separate message threads with no shared record for the office.', 'Send contractor messages from the gig by SMS or WhatsApp, keep the conversation in Contact History, and see the context alongside the booking.'],
...
const featureGroups = [
  ...
  ['communication', 'bolt', 'Contractor Messaging', ['Choose SMS or WhatsApp from the contractor strip', 'A monthly message allowance included with every plan', 'Contact History keeps outreach connected to the gig', 'Usage alerts before the monthly allowance is reached']],
...
const faqItems = [
  ...
  ['Is Contractor Messaging included in my plan?', 'Yes. Solo includes 100 contractor messages per month, Team includes 500, and Studio includes 1,500. Agency includes 500 messages per managed group. You can choose SMS or WhatsApp when messaging launches; email remains included separately.'],
  ...
  ['What counts as one contractor message?', 'A normal-length SMS or regular WhatsApp message counts toward the same monthly allowance. Longer SMS messages can be split into more than one message by mobile carriers, and GigWorks shows the count before you send.'],
...
const comparisonCategories = [
  ...
  ['Contractor messaging', [
    ...
    ['SMS or WhatsApp from the contractor strip', 'Included', 'Included', 'Included'],
...
export const DEFAULT_WEBSITE_CONFIG = {
  ...
  comingSoon: { enabled: true, label: 'Coming soon', heading: 'More ways to run the gig from one place', description: 'These additions are being prepared for launch. Availability and timing may change as we finish testing.', items: [{ header: 'Contractor Messaging', description: 'Message contractors directly from the contractor strip using SMS or WhatsApp. Both channels share the message allowance included with the plan, and conversations stay recorded in Contact History.' }] },
  pricing: { label: 'Simple pricing', heading: 'Every plan runs the whole gig', description: 'Core tools and a monthly Contractor Messaging allowance are built into every plan. Choose based on the size and structure of your operation.', monthlyLabel: 'Monthly', annualLabel: 'Annual', annualSavingsLabel: 'Save up to 20%', featuredLabel: 'Most popular', perMonthLabel: '/month', billedAnnuallyLabel: 'billed annually', billedMonthlyLabel: 'Billed monthly', trialButtonLabel: 'Start {days}-day free trial', trialFooterLabel: '{days}-day free trial at launch.', trialDays: 14, footer: 'Cancel anytime. Secure billing through Stripe. Contractor Messaging requires activation and will be available when SMS and WhatsApp launch.', includedFeatures: ['Unlimited clients, bookings, and events', 'Proposals, e-sign contracts, and invoices', 'Contractor confirmations and availability', 'Stage plots, set lists, and production details', 'Templates, reminders, and client payment tools'], messaging: { heading: 'Need more contractor messages? Add them without changing plans.', description: 'Choose a monthly message package whenever your team needs more contractor communication. Use the same allowance for SMS or WhatsApp.', availabilityLabel: 'Contractor Messaging launching soon', usageNote: 'No surprise overages. Account owners receive alerts at 75%, 90%, and 100%, then choose whether to add more messages.', packages: [{ texts: 500, monthlyAmountCents: 1500 }, { texts: 1000, monthlyAmountCents: 2500 }, { texts: 2500, monthlyAmountCents: 5000 }] }, tiers: [...PLAN_TIERS.map((tier) => ({ id: tier.id, name: tier.label, seatLimit: tier.seatLimit, includedTexts: tier.id === 'solo' ? 100 : tier.id === 'team' ? 500 : 1500, monthlyAmountCents: tier.monthly.amountCents, annualAmountCents: tier.annual.amountCents, description: tier.id === 'solo' ? 'For independent bandleaders and performers running their own calendar.' : tier.id === 'team' ? 'For small teams sharing bookings, staffing, and client follow-up.' : 'For growing entertainment companies coordinating multiple people.', featured: tier.id === 'team' })), { id: 'agency', name: 'Agency', seatLimit: null, includedTexts: 500, includedTextsPerGroup: true, monthlyAmountCents: 14900, annualAmountCents: 142800, includedGroupCount: 2, monthlyAdditionalGroupCents: 3500, annualAdditionalGroupCents: 33600, description: 'For management companies coordinating multiple bands or acts, each with its own workflow and brand.', featured: false }] },
