...
const painItems = [
  ['Confirmed? Tentative? Ghosted?', "Chasing contractors for a yes/no by text, with no single place to see who's actually locked in for Saturday.", "Every contractor's status is tracked per event — they can confirm or decline from a link on their own phone, or just reply to your email and let AI update their status the moment the reply comes in."],
  ...
  ['Contractor conversations disappear into personal phones.', 'Confirmations, schedule changes, and payment follow-ups get scattered across separate message threads with no shared record for the office.', 'Use a dedicated Gigworks number for tracked SMS conversations, or open an external WhatsApp chat when that is more convenient.'],
...
const featureGroups = [
  ['clients', 'file', 'For your clients', ['Inquiry-to-booking pipeline', 'Proposals your client accepts online', 'Contracts with e-signature', 'Invoicing with built-in Stripe payments', 'AI drafts a priced proposal straight from a client’s inquiry']],
  ['roster', 'users', 'For your roster', ['Contractor roster & availability', 'Per-event confirm/decline tracking', 'Ensembles — save a group once, add its whole lineup in one click', 'A home-screen link every contractor can check themselves', 'AI reads a contractor’s email reply and updates their status automatically']],
  ...
  ['communication', 'bolt', 'Contractor Messaging', ['A dedicated Gigworks phone number managed for your business', 'Send and receive operational SMS from the contractor strip', 'Contact History keeps texts, replies, and delivery status connected to the gig', 'Open WhatsApp as an external shortcut when preferred']],
...
function featureGroupsList(input, fallback) {
  ...
  const withMessaging = incoming.some((group) => group?.id === 'communication') ? incoming : [...incoming, fallback.find((group) => group.id === 'communication')].filter(Boolean);
  return withMessaging.slice(0, 8).map((group, i) => {
...
const faqItems = [
  ...
  ['Is Contractor Messaging included in my plan?', 'Yes. Solo includes 100 SMS messages per month, Team includes 500, and Studio includes 1,500. Agency includes 500 per managed group. SMS uses a dedicated Gigworks business number and stays in Contact History; the WhatsApp shortcut opens an external conversation that Gigworks cannot track. Email remains included separately.'],
  ['What happens if I need more contractor messages?', 'You can add a monthly message package without changing plans: 500 additional messages for $15, 1,000 for $25, or 2,500 for $50. GigWorks warns account owners as usage approaches the monthly limit and does not automatically create surprise overage charges.'],
  ['What counts as one contractor message?', 'Each incoming or outgoing SMS segment counts toward the monthly allowance. Longer messages can be split into multiple segments by mobile carriers. WhatsApp shortcut conversations take place outside Gigworks and do not use the allowance.'],
  ['Does GigWorks use AI? What does it actually do, and is my data safe?', "Yes — GigWorks uses AI to read contractor replies and update confirmation status, and in GigWorks Assistant. The Assistant proactively recommends short, permission-aware lessons, remembers each user's progress, answers questions about real work, and can prepare supported changes such as reminders, clients, or booking updates. Anything it proposes is shown to you first — nothing saves until you confirm it. Your account data is only used to answer your own questions or process your own emails through Anthropic's Claude API; it is never shared across accounts or used to train an AI model."],
...
const comparisonCategories = [
  ...
  ['Contractor messaging', [
    ['Monthly contractor messages included', '100', '500', '1,500'],
    ['Dedicated-number SMS from the contractor strip', 'Included', 'Included', 'Included'],
    ['Shared Contact History', 'Included', 'Included', 'Included'],
    ['Usage alerts at 75%, 90%, and 100%', 'Included', 'Included', 'Included'],
  ]],
].map(([name, rows], categoryIndex) => ({
  id: `category-${categoryIndex + 1}`,
  name,
  rows: rows.map(([feature, solo, team, studio], rowIndex) => ({ id: `feature-${categoryIndex + 1}-${rowIndex + 1}`, feature, solo, team, studio, agency: feature === 'Team members' ? 'Custom' : feature === 'Monthly contractor messages included' ? '500 per group' : 'Included' })),
}));
...
export const DEFAULT_WEBSITE_CONFIG = {
  ...
  comingSoon: { enabled: true, label: 'Coming soon', heading: 'More ways to run the gig from one place', description: 'These additions are being prepared for launch. Availability and timing may change as we finish testing.', items: [{ header: 'Contractor Messaging: Dedicated Gigworks Number', description: 'Send and receive operational SMS from contractor strips using a business number managed by Gigworks. Texts, replies, and delivery status stay connected to each gig in Contact History.' }] },
  pricing: { label: 'Simple pricing', heading: 'Every plan runs the whole gig', description: 'Core tools and a monthly Contractor Messaging allowance are built into every plan. Choose based on the size and structure of your operation.', monthlyLabel: 'Monthly', annualLabel: 'Annual', annualSavingsLabel: 'Save up to 20%', featuredLabel: 'Most popular', perMonthLabel: '/month', billedAnnuallyLabel: 'billed annually', billedMonthlyLabel: 'Billed monthly', trialButtonLabel: 'Start {days}-day free trial', trialFooterLabel: '{days}-day free trial at launch.', trialDays: 14, footer: 'Cancel anytime. Secure billing through Stripe. Dedicated-number SMS requires carrier registration and activation.', includedFeatures: ['Unlimited clients, bookings, and events', 'Proposals, e-sign contracts, and invoices', 'Contractor confirmations and availability', 'Stage plots, set lists, and production details', 'Templates, reminders, and client payment tools'], messaging: { heading: 'Need more SMS messages? Add them without changing plans.', description: 'Choose a monthly message package whenever your team needs more tracked contractor communication.', availabilityLabel: 'Dedicated-number SMS launching soon', usageNote: 'No surprise overages. Account owners receive alerts at 75%, 90%, and 100%, then choose whether to add more messages.', packages: [{ texts: 500, monthlyAmountCents: 1500 }, { texts: 1000, monthlyAmountCents: 2500 }, { texts: 2500, monthlyAmountCents: 5000 }] }, tiers: [...PLAN_TIERS.map((tier) => ({ id: tier.id, name: tier.label, seatLimit: tier.seatLimit, includedTexts: tier.id === 'solo' ? 100 : tier.id === 'team' ? 500 : 1500, monthlyAmountCents: tier.monthly.amountCents, annualAmountCents: tier.annual.amountCents, description: tier.id === 'solo' ? 'For independent bandleaders and performers running their own calendar.' : tier.id === 'team' ? 'For small teams sharing bookings, staffing, and client follow-up.' : 'For growing entertainment companies coordinating multiple people.', featured: tier.id === 'team' })), { id: 'agency', name: 'Agency', seatLimit: null, includedTexts: 500, includedTextsPerGroup: true, monthlyAmountCents: 14900, annualAmountCents: 142800, includedGroupCount: 2, monthlyAdditionalGroupCents: 3500, annualAdditionalGroupCents: 33600, description: 'For management companies coordinating multiple bands or acts, each with its own workflow and brand.', featured: false }] },
...
function count(value, fallback, max = 1000000) { const parsed = Number.parseInt(value, 10); return Number.isInteger(parsed) && parsed >= 0 && parsed <= max ? parsed : fallback; }
...
function comparison(input, fallback) {
  ...
  const categoriesWithMessaging = incomingCategories.some((category) => category?.name === 'Contractor messaging')
    ? incomingCategories
    : [...incomingCategories, fallback.categories.find((category) => category.name === 'Contractor messaging')].filter(Boolean);
  return {
    ...
    categories: categoriesWithMessaging.slice(0, 12).map((category, categoryIndex) => {
