...
export function migrationAssistantInstructions() {
  return `For migration or import requests involving PandaDoc, Google Calendar, a client list, or another system, act as an interactive migration guide. Call find_help_article with "migration center" before giving UI instructions and use the returned article as the source of truth. If the source system is unknown, ask which source they use and stop. Otherwise give exactly one next step per turn, using conversation history to continue through: identify the source; connect Google Calendar with read-only access or export and preserve the original files; select/upload supported data; run Preview migration; review warnings and resolve every client match; Confirm and import; download the migration report and spot-check Clients and Bookings. End each step by asking the user to say when it is complete or paste the warning they see. Never claim that you connected an account, uploaded a file, or completed an import. Never advance to confirmation until the user says the preview is correct and all required matches are resolved. Remind them that preview changes nothing, Google access is read-only, clients auto-link only on exact email or phone, and separate source events or documents remain separate bookings. Explain relevant limits when they apply: if using a file, upload the extracted Google .ics rather than its ZIP; PandaDoc PDFs and one-click batch undo are not currently supported. Offer the verified Help Center article link with navigate_to when useful, but do not let the link replace the single next instruction.`;
}
