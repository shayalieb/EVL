...
// anything — every later tick's upsert is a no-op `update: {}` against the
// same row, whether it's still pending, already sent, or already completed.
async function upsertAutoReminder({ accountId, createdByUserId, relatedType, relatedId, relatedName, ruleKey, note }) {
  if (!createdByUserId) return; // no owner membership found — nothing sane to send from
  await prisma.reminder.upsert({
    where: { accountId_relatedType_relatedId_ruleKey: { accountId, relatedType, relatedId, ruleKey } },
    update: {},
    create: {
      accountId,
      createdByUserId,
...
async function createEventReminders() {
  ...
  for (const event of candidates) {
    ...
    await upsertAutoReminder({
      ...
      createdByUserId: ownerUserIdsByAccount.get(event.accountId),
      relatedType: 'event',
      relatedId: event.id,
...
async function createInvoiceReminders() {
  ...
  for (const invoice of candidates) {
    await upsertAutoReminder({
      ...
      createdByUserId: ownerUserIdsByAccount.get(invoice.accountId),
      relatedType: 'invoice',
