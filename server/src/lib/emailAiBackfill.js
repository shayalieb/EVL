...
import { stripQuotedText } from './emailQuoteStrip.js';
import { classifyContractorReply } from './emailReplyClassifier.js';
import { applyAiReplyClassification } from './contractorAiStatusUpdate.js';
...
// Reclassifies inbound EmailMessage rows that never got an AI reply
// classification (aiClassification null — e.g. received while
...
// already-processed rows. Shared by server/scripts/backfillEmailAiClassification.js
// (for local dev, run directly) and the admin-triggered route (for
// production, whose DB is only reachable from inside the running app).
export async function backfillEmailAiClassification() {
