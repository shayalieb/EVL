// ⚠️ SYNCED COPY — the real source of truth is /src/lib/helpArticles.js
// (frontend). This file exists ONLY because Railway's EVL service has its
// root directory set to server/, so a build there can't see anything
// outside it — a plain relative import reaching into the top-level src/
// worked in local dev (same checkout) but crashed production on boot with
// "Cannot find module" the one time it was tried (see git history around
// the GigWorks Assistant's find_help_article tool). If you edit the real
// src/lib/helpArticles.js, copy the change here too, or the Assistant's
// answers will silently drift out of sync with what HelpPage.jsx shows.
//
    ...
    articles: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'The Learn tab shows setup, data migration, booking, staffing, invoicing, contractor payment, and day-of preparation guides. You only see lessons for areas your account permissions allow you to use.' },
      ...
      {
        id: 'migration-center',
        title: 'Migrate your data into GigWorks',
        summary: 'A safe, preview-first walkthrough for PandaDoc, Google Calendar, and client CSV files.',
        blocks: [
          { type: 'p', text: 'Migration Center brings existing clients and booking history into GigWorks without changing your source system. Keep the original exports until you have reviewed the completed migration and its report.' },
          { type: 'h', text: '1. Choose your source' },
          { type: 'list', items: [
            'PandaDoc — use a contacts CSV plus a Document data or Data analytics CSV to preserve clients and document history.',
            'Google Calendar — connect with read-only Google OAuth for direct selection, or use the calendar .ics file; add a client CSV when you also have a separate client list.',
            'Client list only — use a CSV when you only need to bring over contact records.',
          ] },
          { type: 'h', text: '2. Export the files' },
          { type: 'steps', items: [
            'For PandaDoc contacts, ask PandaDoc Support for your workspace contact export. In PandaDoc Reports, export Document data or Data analytics as CSV.',
            'For Google Calendar, choose Connect Google Calendar in Migration Center and approve read-only access. If you prefer a file, open Google Calendar Settings → Import & export → Export, then locate the .ics file inside the ZIP.',
            'For another client system, export a CSV with first name, last name, email, phone, and notes whenever those fields are available.',
            'Save an untouched copy of every export. Do not delete the source data yet.',
          ] },
          { type: 'note', text: 'Google OAuth is read-only: GigWorks cannot edit or delete Google events. If using a file instead, upload the extracted .ics rather than the ZIP. Original signed PandaDoc PDFs are not imported in this phase.' },
          { type: 'h', text: '3. Upload and preview' },
          { type: 'steps', items: [
            'Open Import Data to enter the Migration Center.',
            'Choose PandaDoc, Google Calendar, or Client list.',
            'Give the migration a recognizable name and attach the available files.',
            'Choose “Preview migration.” Nothing is added to GigWorks during preview.',
          ] },
          { type: 'h', text: '4. Review client matches' },
          { type: 'p', text: 'GigWorks automatically links a client only when the email address or phone number is an exact match. A matching name alone is never enough to merge records.' },
          { type: 'steps', items: [
            'Read the summary and every warning.',
            'For each possible client match, choose the correct existing client, create a new client, or skip that row.',
            'Resolve every required match before continuing. The confirmation button remains unavailable while a decision is still needed.',
          ] },
          { type: 'h', text: '5. Check booking history' },
          { type: 'list', items: [
            'Every separate Google Calendar event remains a separate booking, even when one client has many events.',
            'Every separate PandaDoc document remains a separate booking with its available status, dates, amount, currency, source ID, and original link.',
            'Past calendar events and completed source records go to Completed Bookings. Upcoming or open records stay in the active workflow.',
          ] },
          { type: 'h', text: '6. Confirm and verify' },
          { type: 'steps', items: [
            'When the preview is correct and all matches are resolved, choose “Confirm and import.”',
            'Wait for the completion summary, then choose “Download migration report.” Keep this CSV with your source exports.',
            'Open Clients and Bookings and spot-check several records, including a client with multiple separate bookings and at least one past event.',
          ] },
          { type: 'tip', text: 'If the preview does not look right, stop there and correct the source file. Preview does not write any records, so you can safely try again.' },
          { type: 'note', text: 'Recurring calendar rules, ZIP/PDF ingestion, and one-click batch undo are not currently supported. Source IDs help GigWorks recognize a repeated import; files without stable source IDs may need extra duplicate review.' },
        ],
      },
      {
        id: 'setup',
    ...
    articles: [
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'A new account starts with clear empty states on both lists. Use "+ Add Client" or "+ Add Venue" when you want to build the lists by hand; while editing a booking or event, the searchable Client and Venue fields also let you reuse a saved record without loading the entire list.' },
          { type: 'p', text: 'You rarely need to add a client by hand, though — reviewing an inquiry (see Turn an Inquiry Into a Booking) creates one automatically when no similar client exists. When a possible duplicate is found, GigWorks pauses and lets you compare before linking or creating anyone.' },
          { type: 'p', text: 'Venue works the same way during booking and event entry: choose a saved venue to copy its address/contact details, or keep typing a new venue name and details to save it for reuse.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'steps', items: [
            ...
            'If no similar client exists, GigWorks creates a new client automatically and links it to the booking.',
            'If an email, phone number, or similar name may belong to an existing client, compare the inquiry with the suggested records and choose the correct client or "Create a new client."',
            'If the inquiry came from a Booking that already has a client, that relationship is preserved; the response updates the Booking without replacing or duplicating its client.',
          ...
          { type: 'image', src: '/help/inquiry-client-new.png', alt: 'Inquiry Response showing that no similar client was found and a new client will be created automatically', caption: 'No similar client: applying creates and links a new client automatically.' },
          { type: 'image', src: '/help/inquiry-client-compare.png', alt: 'Inquiry Response showing possible existing clients with exact-email and similar-name reasons plus a Create a new client choice', caption: 'Possible match: compare the contact details and choose explicitly before applying.' },
          { type: 'image', src: '/help/inquiry-client-linked.png', alt: 'Inquiry Response showing that the booking is already linked to a client and that relationship will be preserved', caption: 'Already linked: applying updates the Booking and keeps its existing client relationship.' },
          { type: 'note', text: 'GigWorks never merges two client records automatically. The review choice only decides which client the inquiry and Booking should use.' },
        ],
      },
      {
        id: 'proposals',
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'list', items: [
            ...
            'Regular icons resize proportionally from their corner handles. Select one to enter an exact size and rotation, or resize several selected icons together.',
            'Undo/Redo, zoom controls, "Center All," and "Fit Stage" live in the toolbar.',
          ] },
          { type: 'h', text: 'Define the real stage' },
          { type: 'p', text: 'Open “Stage setup, dimensions and venue plan” to enter the stage width and depth, choose feet or meters, mark which edge faces the audience, and choose a rectangular, rounded, or thrust layout. You can show a measurement grid, center line, and editing-only print-safe guide.' },
          { type: 'p', text: 'Frequently used club, ballroom, festival, and theater sizes are available as starting presets. Named zones can identify areas such as monitor world, FOH, backstage, or restricted space, with exact positions and dimensions.' },
          { type: 'p', text: 'A PNG, JPG, or WebP venue plan can be added behind the stage and faded with the opacity control. GigWorks optimizes the image before saving it. Riser icons also accept exact real-world width and depth.' },
          { type: 'h', text: 'The Production List' },
          { type: 'p', text: "Select an icon and choose “Add Item for Selected Icon” when it should appear in the Production List below the canvas — who's playing, their instrument, whether they need 48V phantom power or AC power, and notes. The deliberate link keeps decorative or reference objects out of the input list. Click an icon to jump to its row, or the row's icon button to jump back to the canvas." },
          ...
          { type: 'tip', text: "Use the Email button to send the stage plot, Production List, and Backline List straight to your sound engineer or venue — check whichever sections you want included, and it also attaches a PDF. Selection outlines and editing handles are automatically removed from the sent images and PDF." },
          { type: 'h', text: 'Reusing a plot for a similar gig' },
          { type: 'p', text: 'Playing the same venue again, with the same lineup? Click "Save to Library" to save this event\'s canvas, Production List, and Backline List for reuse. You can also open Stage Plot Library from the main navigation and click "+ Add Stage Plot" to build a reusable template from scratch, with no event attached.' },
          { type: 'image', src: '/help/stage-plot-library.png', alt: 'The standalone Stage Plot Library listing reusable templates with page, channel, and backline counts', caption: 'The standalone Stage Plot Library — build templates here or save them from a gig.' },
          { type: 'p', text: 'On a gig, click "+ Add from Library," then choose "Add alongside current work" or "Replace selected sections." You can include any combination of Canvas pages, Production list, and Backline list. Replace permanently clears only the sections you checked; append keeps the current work and adds the selected template sections alongside it.' },
          { type: 'image', src: '/help/stage-plot-library-import.png', alt: 'Add from Stage Plot Library dialog with append or replace choices and checkboxes for Canvas pages, Production list, and Backline list', caption: 'Choose append or replace, then include only the sections this gig needs.' },
          ...
          { type: 'note', text: 'Stage plots autosave. If the same page was changed somewhere else, "Updated elsewhere — reload" appears instead of silently overwriting the newer work.' },
        ],
      },
      {
        id: 'set-lists',
    ...
    articles: [
      {
        ...
        blocks: [
          ...
          { type: 'p', text: 'To receive new emails, open Settings → Email Domain → Use your own domain. Connect your domain, add the sending and Inbox receiving DNS records at your provider, then check status. If an existing custom domain has no receiving records, choose Set up Inbox receiving to generate them. Inbox displays an address you can share. Use a dedicated subdomain to preserve mail delivered to your existing Google or Microsoft inbox.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'image', src: '/help/reminder-create-recurring.png', alt: 'The Add Reminder form with related-record search, quick date buttons, timezone, repeat frequency, optional repeat end date, note, and email notification fields', caption: 'Create a standalone or related reminder, then optionally make it recurring.' },
