...
router.post('/send', requireEmailSendPermission, emailSendLimiter, asyncHandler(async (req, res) => {
  ...
  // Availability inquiries should be actionable from the email itself. The
  // persistent contractor calendar remains useful as a portal, while these
  ...
  if (actionBlock) {
    const owner = await prisma.membership.findFirst({ where: { accountId, role: 'owner' } });
    if (owner) {
      const remindAt = new Date(Date.now() + 2 * 24 * 60 * 60 * 1000);
      await prisma.reminder.upsert({
        where: { accountId_relatedType_relatedId_ruleKey: { accountId, relatedType: 'event', relatedId: eventId, ruleKey: `contractor-response:${contractorId}` } },
        update: { remindAt, completedAt: null, emailSentAt: null, emailClaimedAt: null, emailLastFailedAt: null },
        create: {
          accountId,
          ...
          autoGenerated: true,
          ruleKey: `contractor-response:${contractorId}`,
          note: `No contractor response has been recorded after 48 hours. Follow up or find a replacement.`,
          remindAt,
          emailEnabled: true,
        },
      });
  ...
  res.json({ ok: true, threadId: thread.id, messageId: message.id, resendId: sent.data?.id, replyTrackingActive: !!thread.replyToAlias, responseActionsIncluded: !!actionBlock });
}));
