...
test('website configuration preserves safe editable pricing and copy', () => {
  ...
  assert.equal(config.pricing.tiers[0].includedTexts, 100);
  assert.equal(config.pricing.messaging.packages[0].texts, 500);
  ...
  assert.equal(config.painPoints.items.length, 7);
  assert.equal(config.features.groups.length, 5);
  assert.equal(config.features.comparison.categories.length, 5);
  ...
  assert.equal(config.comingSoon.enabled, true);
  assert.equal(config.comingSoon.items.length, 1);
  assert.match(config.comingSoon.items[0].header, /SMS/i);
